      ...
      {data.map((item, index) => (
        <div key={index} className="space-y-1">
          <div className="flex items-center justify-between text-xs">
            <span className="font-medium text-(--text-secondary)">{item.day}</span>
            <span className="text-(--foreground)">{item.you + item.partner} total</span>
          </div>
          <div className="flex gap-1.5">
            <div className="relative h-6 flex-1 overflow-hidden rounded bg-(--border)">
              <div
            ...
            <div className="relative h-6 flex-1 overflow-hidden rounded bg-(--border)">
              <div
                className="h-full bg-(--partner-color) transition-all"
